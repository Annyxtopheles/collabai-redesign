  ...
  container.innerHTML = `
    ...
    <div class="fixed bottom-14 left-2.5 w-84 bg-app-surface border border-app-borderSubtle rounded-2xl p-2.5 shadow-2xl z-50 flex flex-col gap-2 text-[12.5px] font-normal select-none animate-fade-in" onclick="event.stopPropagation()">
      ...
      <div class="p-2.5 bg-app-elevated rounded-xl flex flex-col gap-2 border border-app-borderSubtle">
        ...
        <!-- Dynamically Scaled Theme Grid (Supports 6 Themes: Dark, Light, Pink, CRT, Synth, SaaS) -->
        <div class="grid grid-cols-6 gap-0.5 bg-app-input p-1 rounded-xl border border-app-borderSubtle text-[9.5px]">
          ${themes.map(t => {
            ...
            } else if (t.id === 'saas' && isActive) {
              activeStyle = 'bg-[#315EFF] text-white border border-[#315EFF] font-semibold shadow-[0_0_10px_rgba(49,94,255,0.45)]';
            }
            return `
              <button 
                onclick="switchTheme('${t.id}')" 
                title="${escapeHtml(t.description || t.name)}"
                class="flex flex-col items-center justify-center gap-0.5 py-1 px-0.5 rounded-lg transition-all ${isActive ? activeStyle : 'text-app-textMuted hover:text-app-textPrimary hover:bg-app-hover'}">
                <i data-lucide="${t.icon || 'palette'}" class="w-3.5 h-3.5"></i>
                <span class="truncate text-[9.5px]">${escapeHtml(t.shortName || t.name)}</span>
              </button>
            `;
          }).join('')}
        </div>

        <!-- Theme Ambient Background Effects Control (Indigo Aurora, Synth Horizon, CRT Raster, Vortex/Glyph/Stars, Sakura) -->
        <div class="flex flex-col gap-1.5 pt-1.5 border-t border-app-borderSubtle text-[11.5px]">
          <div class="flex items-center justify-between px-0.5">
            <div class="flex items-center gap-1.5 text-app-textSecondary">
              <i data-lucide="${currentTheme === 'saas' ? 'sparkle' : (currentTheme === 'synthwave' ? 'zap' : (currentTheme === 'crt' ? 'tv' : (currentTheme === 'pink' ? 'flower-2' : (currentTheme === 'dark' ? (appStore.state.darkAmbientStyle === 'stars' ? 'sparkles' : (appStore.state.darkAmbientStyle === 'matrix' ? 'binary' : 'orbit')) : 'binary'))))}" class="w-3.5 h-3.5 ${ambientEnabled ? (currentTheme === 'saas' ? 'text-blue-400' : (currentTheme === 'synthwave' ? 'text-pink-400' : (currentTheme === 'crt' ? 'text-emerald-400' : (currentTheme === 'pink' ? 'text-pink-400' : 'text-emerald-400')))) : 'text-app-textMuted'}"></i>
          ...
          ${ambientEnabled && currentTheme === 'dark' ? `
            <div class="grid grid-cols-4 gap-1 bg-app-input p-1 rounded-lg border border-app-borderSubtle text-[10px]">
              ...
              <button 
                ...
                <i data-lucide="sparkles" class="w-3 h-3 text-cyan-300"></i>
...
// React Bits - Border Glow Pointer Physics Handler (for Dark, CRT, Synthwave, SaaS)
function initBorderGlowPhysics() {
  document.addEventListener('pointermove', (e) => {
    const theme = appStore.state.theme;
    if (theme === 'light' || theme === 'pink') return;
